    ...
    def connect_toggle(self) -> None:
        ...
        if not self.ser.isOpen():
            if self.ser_com_connect():
                ...
                # Set appropriate defaults for simulation mode
                current_index = self.port_combo_box.currentIndex()
                com_port = self.port_combo_box.itemData(current_index)
                if com_port == "SIM":
                    # Set a larger holdoff value for simulation mode to avoid false detections
                    self.holdoff_box.setValue(500)
                    # Set higher prominence for better peak detection
                    self.prominence_box.setValue(250)
                    # Disable auto-holdoff for simulation mode
                    self.actionAuto_Holdoff.setChecked(False)
                    # Reset the graph to clean state
                    self.reset()
