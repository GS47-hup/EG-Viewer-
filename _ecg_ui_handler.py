...
def ui_run_toggle(self) -> None:
    ...
    # Check if we're in simulation mode or if the serial port is open
    is_simulation = current_port == "SIM"
    is_port_open = getattr(self.ser, 'isOpen', lambda: False)()
    
    if not (is_simulation or is_port_open):
        logging.error("Serial port is not open and not in simulation mode")
        return
