# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'ecg_viewer_window.ui'
#
# Created by: PyQt5 UI code generator 5.15.11
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        MainWindow.setObjectName("MainWindow")
        MainWindow.resize(847, 625)
        MainWindow.setMinimumSize(QtCore.QSize(847, 625))
        MainWindow.setBaseSize(QtCore.QSize(800, 750))
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap("icon/icon.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        MainWindow.setWindowIcon(icon)
        self.centralwidget = QtWidgets.QWidget(MainWindow)
        self.centralwidget.setObjectName("centralwidget")
        self.gridLayout_5 = QtWidgets.QGridLayout(self.centralwidget)
        self.gridLayout_5.setObjectName("gridLayout_5")
        self.gridFrame = QtWidgets.QFrame(self.centralwidget)
        self.gridFrame.setObjectName("gridFrame")
        self.gridLayout = QtWidgets.QGridLayout(self.gridFrame)
        self.gridLayout.setContentsMargins(-1, 0, -1, 0)
        self.gridLayout.setSpacing(0)
        self.gridLayout.setObjectName("gridLayout")
        self.frame = QtWidgets.QFrame(self.gridFrame)
        self.frame.setFrameShape(QtWidgets.QFrame.Panel)
        self.frame.setObjectName("frame")
        self.gridLayout_7 = QtWidgets.QHBoxLayout(self.frame)
        self.gridLayout_7.setSpacing(0)
        self.gridLayout_7.setObjectName("gridLayout_7")
        self.gridLayout_2 = QtWidgets.QGridLayout()
        self.gridLayout_2.setObjectName("gridLayout_2")
        self.button_connect = QtWidgets.QPushButton(self.frame)
        self.button_connect.setMaximumSize(QtCore.QSize(16777215, 50))
        self.button_connect.setObjectName("button_connect")
        self.gridLayout_2.addWidget(self.button_connect, 4, 1, 1, 1)
        self.button_refresh = QtWidgets.QPushButton(self.frame)
        self.button_refresh.setMaximumSize(QtCore.QSize(16777215, 50))
        self.button_refresh.setObjectName("button_refresh")
        self.gridLayout_2.addWidget(self.button_refresh, 4, 0, 1, 1)
        self.port_combo_box = QtWidgets.QComboBox(self.frame)
        self.port_combo_box.setObjectName("port_combo_box")
        self.gridLayout_2.addWidget(self.port_combo_box, 3, 0, 1, 2)
        self.label_2 = QtWidgets.QLabel(self.frame)
        self.label_2.setObjectName("label_2")
        self.gridLayout_2.addWidget(self.label_2, 2, 0, 1, 2)
        self.gridLayout_7.addLayout(self.gridLayout_2)
        self.gridLayout.addWidget(self.frame, 2, 0, 1, 1)
        self.gridFrame_2 = QtWidgets.QFrame(self.gridFrame)
        self.gridFrame_2.setMaximumSize(QtCore.QSize(441, 90))
        self.gridFrame_2.setFrameShape(QtWidgets.QFrame.Box)
        self.gridFrame_2.setObjectName("gridFrame_2")
        self.gridLayout_3 = QtWidgets.QGridLayout(self.gridFrame_2)
        self.gridLayout_3.setObjectName("gridLayout_3")
        self.high_limit_box = QtWidgets.QSpinBox(self.gridFrame_2)
        self.high_limit_box.setMinimumSize(QtCore.QSize(0, 24))
        self.high_limit_box.setMinimum(85)
        self.high_limit_box.setMaximum(150)
        self.high_limit_box.setProperty("value", 120)
        self.high_limit_box.setObjectName("high_limit_box")
        self.gridLayout_3.addWidget(self.high_limit_box, 1, 1, 1, 1)
        self.label_3 = QtWidgets.QLabel(self.gridFrame_2)
        self.label_3.setObjectName("label_3")
        self.gridLayout_3.addWidget(self.label_3, 0, 0, 1, 1)
        self.label_4 = QtWidgets.QLabel(self.gridFrame_2)
        self.label_4.setMinimumSize(QtCore.QSize(0, 24))
        self.label_4.setObjectName("label_4")
        self.gridLayout_3.addWidget(self.label_4, 1, 0, 1, 1)
        self.label_5 = QtWidgets.QLabel(self.gridFrame_2)
        self.label_5.setMinimumSize(QtCore.QSize(0, 24))
        self.label_5.setObjectName("label_5")
        self.gridLayout_3.addWidget(self.label_5, 2, 0, 1, 1)
        self.low_limit_box = QtWidgets.QSpinBox(self.gridFrame_2)
        self.low_limit_box.setMinimumSize(QtCore.QSize(0, 24))
        self.low_limit_box.setMinimum(15)
        self.low_limit_box.setMaximum(70)
        self.low_limit_box.setProperty("value", 40)
        self.low_limit_box.setObjectName("low_limit_box")
        self.gridLayout_3.addWidget(self.low_limit_box, 2, 1, 1, 1)
        self.gridLayout.addWidget(self.gridFrame_2, 3, 0, 1, 1)
        self.alarm_window = QtWidgets.QFrame(self.gridFrame)
        self.alarm_window.setMinimumSize(QtCore.QSize(370, 90))
        self.alarm_window.setMaximumSize(QtCore.QSize(16777215, 90))
        self.alarm_window.setStyleSheet("QFrame { background-color: red }")
        self.alarm_window.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.alarm_window.setFrameShadow(QtWidgets.QFrame.Raised)
        self.alarm_window.setObjectName("alarm_window")
        self.gridLayout_6 = QtWidgets.QGridLayout(self.alarm_window)
        self.gridLayout_6.setObjectName("gridLayout_6")
        self.alarm_text = QtWidgets.QLabel(self.alarm_window)
        font = QtGui.QFont()
        font.setPointSize(20)
        font.setBold(True)
        font.setWeight(75)
        self.alarm_text.setFont(font)
        self.alarm_text.setScaledContents(True)
        self.alarm_text.setAlignment(QtCore.Qt.AlignCenter)
        self.alarm_text.setObjectName("alarm_text")
        self.gridLayout_6.addWidget(self.alarm_text, 0, 0, 1, 1)
        self.gridLayout.addWidget(self.alarm_window, 3, 1, 1, 1)
        self.label = QtWidgets.QLabel(self.gridFrame)
        font = QtGui.QFont()
        font.setBold(True)
        font.setWeight(75)
        self.label.setFont(font)
        self.label.setAlignment(QtCore.Qt.AlignCenter)
        self.label.setObjectName("label")
        self.gridLayout.addWidget(self.label, 1, 1, 1, 1)
        spacerItem = QtWidgets.QSpacerItem(40, 20, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum)
        self.gridLayout.addItem(spacerItem, 2, 2, 1, 1)
        self.lcdNumber = QtWidgets.QLCDNumber(self.gridFrame)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(self.lcdNumber.sizePolicy().hasHeightForWidth())
        self.lcdNumber.setSizePolicy(sizePolicy)
        self.lcdNumber.setMinimumSize(QtCore.QSize(370, 105))
        self.lcdNumber.setFrameShape(QtWidgets.QFrame.Panel)
        self.lcdNumber.setFrameShadow(QtWidgets.QFrame.Plain)
        self.lcdNumber.setObjectName("lcdNumber")
        self.gridLayout.addWidget(self.lcdNumber, 2, 1, 1, 1)
        self.horizontalLayout = QtWidgets.QHBoxLayout()
        self.horizontalLayout.setSpacing(6)
        self.horizontalLayout.setObjectName("horizontalLayout")
        self.show_track = QtWidgets.QCheckBox(self.gridFrame)
        self.show_track.setMinimumSize(QtCore.QSize(200, 20))
        self.show_track.setToolTip("")
        self.show_track.setToolTipDuration(-1)
        self.show_track.setWhatsThis("")
        self.show_track.setObjectName("show_track")
        self.horizontalLayout.addWidget(self.show_track)
        self.button_reset = QtWidgets.QPushButton(self.gridFrame)
        self.button_reset.setObjectName("button_reset")
        self.horizontalLayout.addWidget(self.button_reset)
        self.button_run = QtWidgets.QPushButton(self.gridFrame)
        self.button_run.setObjectName("button_run")
        self.horizontalLayout.addWidget(self.button_run)
        self.gridLayout.addLayout(self.horizontalLayout, 1, 0, 1, 1)
        self.gridLayout_5.addWidget(self.gridFrame, 5, 0, 1, 2)
        self.gridFrame_21 = QtWidgets.QFrame(self.centralwidget)
        self.gridFrame_21.setMinimumSize(QtCore.QSize(200, 0))
        self.gridFrame_21.setMaximumSize(QtCore.QSize(200, 16777215))
        self.gridFrame_21.setFrameShape(QtWidgets.QFrame.Box)
        self.gridFrame_21.setObjectName("gridFrame_21")
        self.gridLayout_4 = QtWidgets.QGridLayout(self.gridFrame_21)
        self.gridLayout_4.setContentsMargins(0, 0, 0, 0)
        self.gridLayout_4.setObjectName("gridLayout_4")
        self.verticalLayout = QtWidgets.QVBoxLayout()
        self.verticalLayout.setSpacing(0)
        self.verticalLayout.setObjectName("verticalLayout")
        self.label_6 = QtWidgets.QLabel(self.gridFrame_21)
        self.label_6.setMinimumSize(QtCore.QSize(0, 0))
        self.label_6.setMaximumSize(QtCore.QSize(200, 20))
        font = QtGui.QFont()
        font.setBold(True)
        font.setWeight(75)
        self.label_6.setFont(font)
        self.label_6.setFrameShape(QtWidgets.QFrame.NoFrame)
        self.label_6.setLineWidth(0)
        self.label_6.setAlignment(QtCore.Qt.AlignCenter)
        self.label_6.setObjectName("label_6")
        self.verticalLayout.addWidget(self.label_6)
        spacerItem1 = QtWidgets.QSpacerItem(20, 10, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Fixed)
        self.verticalLayout.addItem(spacerItem1)
        self.label_8 = QtWidgets.QLabel(self.gridFrame_21)
        self.label_8.setObjectName("label_8")
        self.verticalLayout.addWidget(self.label_8)
        self.holdoff_box = QtWidgets.QSpinBox(self.gridFrame_21)
        self.holdoff_box.setEnabled(False)
        self.holdoff_box.setMinimumSize(QtCore.QSize(172, 24))
        self.holdoff_box.setToolTip("")
        self.holdoff_box.setToolTipDuration(-1)
        self.holdoff_box.setMinimum(100)
        self.holdoff_box.setMaximum(2000)
        self.holdoff_box.setSingleStep(100)
        self.holdoff_box.setStepType(QtWidgets.QAbstractSpinBox.DefaultStepType)
        self.holdoff_box.setProperty("value", 600)
        self.holdoff_box.setObjectName("holdoff_box")
        self.verticalLayout.addWidget(self.holdoff_box)
        self.label_9 = QtWidgets.QLabel(self.gridFrame_21)
        self.label_9.setObjectName("label_9")
        self.verticalLayout.addWidget(self.label_9)
        self.prominence_box = QtWidgets.QSpinBox(self.gridFrame_21)
        self.prominence_box.setMinimumSize(QtCore.QSize(0, 24))
        self.prominence_box.setProperty("value", 15)
        self.prominence_box.setObjectName("prominence_box")
        self.verticalLayout.addWidget(self.prominence_box)
        self.label_10 = QtWidgets.QLabel(self.gridFrame_21)
        self.label_10.setObjectName("label_10")
        self.verticalLayout.addWidget(self.label_10)
        self.window_length_box = QtWidgets.QSpinBox(self.gridFrame_21)
        self.window_length_box.setMinimumSize(QtCore.QSize(0, 24))
        self.window_length_box.setMaximum(499)
        self.window_length_box.setSingleStep(2)
        self.window_length_box.setProperty("value", 199)
        self.window_length_box.setObjectName("window_length_box")
        self.verticalLayout.addWidget(self.window_length_box)
        self.label_11 = QtWidgets.QLabel(self.gridFrame_21)
        self.label_11.setObjectName("label_11")
        self.verticalLayout.addWidget(self.label_11)
        self.polyorder_box = QtWidgets.QSpinBox(self.gridFrame_21)
        self.polyorder_box.setMinimumSize(QtCore.QSize(0, 24))
        self.polyorder_box.setProperty("value", 7)
        self.polyorder_box.setObjectName("polyorder_box")
        self.verticalLayout.addWidget(self.polyorder_box)
        self.autoinvert_checkbox = QtWidgets.QCheckBox(self.gridFrame_21)
        self.autoinvert_checkbox.setEnabled(True)
        self.autoinvert_checkbox.setMinimumSize(QtCore.QSize(0, 24))
        self.autoinvert_checkbox.setChecked(True)
        self.autoinvert_checkbox.setObjectName("autoinvert_checkbox")
        self.verticalLayout.addWidget(self.autoinvert_checkbox)
        self.button_ui_force_invert = QtWidgets.QPushButton(self.gridFrame_21)
        self.button_ui_force_invert.setMinimumSize(QtCore.QSize(195, 32))
        self.button_ui_force_invert.setObjectName("button_ui_force_invert")
        self.verticalLayout.addWidget(self.button_ui_force_invert)
        self.label_7 = QtWidgets.QLabel(self.gridFrame_21)
        self.label_7.setMinimumSize(QtCore.QSize(183, 24))
        self.label_7.setObjectName("label_7")
        self.verticalLayout.addWidget(self.label_7)
        self.graph_zoom_slider = QtWidgets.QSlider(self.gridFrame_21)
        self.graph_zoom_slider.setMinimum(0)
        self.graph_zoom_slider.setMaximum(99)
        self.graph_zoom_slider.setSingleStep(1)
        self.graph_zoom_slider.setProperty("value", 66)
        self.graph_zoom_slider.setOrientation(QtCore.Qt.Horizontal)
        self.graph_zoom_slider.setInvertedAppearance(True)
        self.graph_zoom_slider.setInvertedControls(False)
        self.graph_zoom_slider.setTickPosition(QtWidgets.QSlider.NoTicks)
        self.graph_zoom_slider.setObjectName("graph_zoom_slider")
        self.verticalLayout.addWidget(self.graph_zoom_slider)
        self.gridLayout_4.addLayout(self.verticalLayout, 0, 0, 1, 1)
        spacerItem2 = QtWidgets.QSpacerItem(20, 40, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding)
        self.gridLayout_4.addItem(spacerItem2, 3, 0, 1, 1)
        self.gridLayout_5.addWidget(self.gridFrame_21, 1, 2, 1, 1)
        self.graph = PlotWidget(self.centralwidget)
        self.graph.setObjectName("graph")
        self.gridLayout_5.addWidget(self.graph, 1, 0, 3, 2)
        self.label_12 = QtWidgets.QLabel(self.centralwidget)
        self.label_12.setMaximumSize(QtCore.QSize(832, 16))
        font = QtGui.QFont()
        font.setPointSize(13)
        font.setBold(True)
        font.setWeight(75)
        self.label_12.setFont(font)
        self.label_12.setAlignment(QtCore.Qt.AlignCenter)
        self.label_12.setObjectName("label_12")
        self.gridLayout_5.addWidget(self.label_12, 7, 0, 1, 3)
        self.simulatorControlsGroup = QtWidgets.QGroupBox(self.centralwidget)
        sizePolicySim = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Maximum)
        self.simulatorControlsGroup.setSizePolicy(sizePolicySim)
        self.simulatorControlsGroup.setObjectName("simulatorControlsGroup")
        self.simulatorLayout = QtWidgets.QVBoxLayout(self.simulatorControlsGroup)
        self.simulatorLayout.setObjectName("simulatorLayout")
        self.dataSourceLabel = QtWidgets.QLabel(self.simulatorControlsGroup)
        self.dataSourceLabel.setObjectName("dataSourceLabel")
        self.simulatorLayout.addWidget(self.dataSourceLabel)
        self.dataSourceGroup = QtWidgets.QButtonGroup(MainWindow)
        self.useSerialRadio = QtWidgets.QRadioButton(self.simulatorControlsGroup)
        self.useSerialRadio.setObjectName("useSerialRadio")
        self.useSerialRadio.setChecked(True)
        self.dataSourceGroup.addButton(self.useSerialRadio)
        self.simulatorLayout.addWidget(self.useSerialRadio)
        self.useGeneratorRadio = QtWidgets.QRadioButton(self.simulatorControlsGroup)
        self.useGeneratorRadio.setObjectName("useGeneratorRadio")
        self.dataSourceGroup.addButton(self.useGeneratorRadio)
        self.simulatorLayout.addWidget(self.useGeneratorRadio)
        self.simulatorLayout.addSpacerItem(QtWidgets.QSpacerItem(20, 10, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding))
        self.ecgTypeLabel = QtWidgets.QLabel(self.simulatorControlsGroup)
        self.ecgTypeLabel.setObjectName("ecgTypeLabel")
        self.simulatorLayout.addWidget(self.ecgTypeLabel)
        self.ecgTypeCombo = QtWidgets.QComboBox(self.simulatorControlsGroup)
        self.ecgTypeCombo.setObjectName("ecgTypeCombo")
        self.simulatorLayout.addWidget(self.ecgTypeCombo)
        self.simulatorLayout.addSpacerItem(QtWidgets.QSpacerItem(20, 10, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding))
        self.heartRateLayout = QtWidgets.QHBoxLayout()
        self.heartRateLabel = QtWidgets.QLabel(self.simulatorControlsGroup)
        self.heartRateLabel.setObjectName("heartRateLabel")
        self.heartRateLayout.addWidget(self.heartRateLabel)
        self.currentHeartRateLabel = QtWidgets.QLabel(self.simulatorControlsGroup)
        self.currentHeartRateLabel.setObjectName("currentHeartRateLabel")
        self.currentHeartRateLabel.setMinimumWidth(40)
        self.currentHeartRateLabel.setAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignVCenter)
        self.heartRateLayout.addWidget(self.currentHeartRateLabel)
        self.simulatorLayout.addLayout(self.heartRateLayout)
        self.heartRateSlider = QtWidgets.QSlider(QtCore.Qt.Horizontal, self.simulatorControlsGroup)
        self.heartRateSlider.setObjectName("heartRateSlider")
        self.heartRateSlider.setMinimum(30)
        self.heartRateSlider.setMaximum(180)
        self.heartRateSlider.setValue(75)
        self.simulatorLayout.addWidget(self.heartRateSlider)
        self.simulatorLayout.addSpacerItem(QtWidgets.QSpacerItem(20, 10, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding))
        self.noiseLayout = QtWidgets.QHBoxLayout()
        self.noiseLabel = QtWidgets.QLabel(self.simulatorControlsGroup)
        self.noiseLabel.setObjectName("noiseLabel")
        self.noiseLayout.addWidget(self.noiseLabel)
        self.currentNoiseLabel = QtWidgets.QLabel(self.simulatorControlsGroup)
        self.currentNoiseLabel.setObjectName("currentNoiseLabel")
        self.currentNoiseLabel.setMinimumWidth(40)
        self.currentNoiseLabel.setAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignVCenter)
        self.noiseLayout.addWidget(self.currentNoiseLabel)
        self.simulatorLayout.addLayout(self.noiseLayout)
        self.noiseSlider = QtWidgets.QSlider(QtCore.Qt.Horizontal, self.simulatorControlsGroup)
        self.noiseSlider.setObjectName("noiseSlider")
        self.noiseSlider.setMinimum(0)
        self.noiseSlider.setMaximum(100)
        self.noiseSlider.setValue(10)
        self.simulatorLayout.addWidget(self.noiseSlider)
        self.gridLayout_5.addWidget(self.simulatorControlsGroup, 2, 2, 2, 1)
        MainWindow.setCentralWidget(self.centralwidget)
        self.statusBar = QtWidgets.QStatusBar(MainWindow)
        self.statusBar.setObjectName("statusBar")
        MainWindow.setStatusBar(self.statusBar)
        self.menuBar = QtWidgets.QMenuBar(MainWindow)
        self.menuBar.setGeometry(QtCore.QRect(0, 0, 847, 24))
        self.menuBar.setObjectName("menuBar")
        self.menuFile = QtWidgets.QMenu(self.menuBar)
        self.menuFile.setObjectName("menuFile")
        self.menuExport = QtWidgets.QMenu(self.menuBar)
        self.menuExport.setObjectName("menuExport")
        self.menuRefresh_Rate = QtWidgets.QMenu(self.menuExport)
        self.menuRefresh_Rate.setObjectName("menuRefresh_Rate")
        self.menuHelp = QtWidgets.QMenu(self.menuBar)
        self.menuHelp.setObjectName("menuHelp")
        self.menuExport_2 = QtWidgets.QMenu(self.menuBar)
        self.menuExport_2.setObjectName("menuExport_2")
        self.menuCapture = QtWidgets.QMenu(self.menuBar)
        self.menuCapture.setObjectName("menuCapture")
        self.menuWindow_Size = QtWidgets.QMenu(self.menuCapture)
        self.menuWindow_Size.setObjectName("menuWindow_Size")
        MainWindow.setMenuBar(self.menuBar)
        self.actionQuit = QtWidgets.QAction(MainWindow)
        self.actionQuit.setObjectName("actionQuit")
        self.actionAbout = QtWidgets.QAction(MainWindow)
        self.actionAbout.setObjectName("actionAbout")
        self.actionBold_Line = QtWidgets.QAction(MainWindow)
        self.actionBold_Line.setCheckable(True)
        self.actionBold_Line.setChecked(True)
        self.actionBold_Line.setObjectName("actionBold_Line")
        self.actionPNG = QtWidgets.QAction(MainWindow)
        self.actionPNG.setObjectName("actionPNG")
        self.actionCSV = QtWidgets.QAction(MainWindow)
        self.actionCSV.setObjectName("actionCSV")
        self.actionRAW = QtWidgets.QAction(MainWindow)
        self.actionRAW.setObjectName("actionRAW")
        self.actionGet_Source_Code = QtWidgets.QAction(MainWindow)
        self.actionGet_Source_Code.setObjectName("actionGet_Source_Code")
        self.actionCapture_Rate = QtWidgets.QAction(MainWindow)
        self.actionCapture_Rate.setObjectName("actionCapture_Rate")
        self.actionReset = QtWidgets.QAction(MainWindow)
        self.actionReset.setObjectName("actionReset")
        self.actionStart_Stop = QtWidgets.QAction(MainWindow)
        self.actionStart_Stop.setObjectName("actionStart_Stop")
        self.actionAuto_Holdoff = QtWidgets.QAction(MainWindow)
        self.actionAuto_Holdoff.setCheckable(True)
        self.actionAuto_Holdoff.setChecked(True)
        self.actionAuto_Holdoff.setObjectName("actionAuto_Holdoff")
        self.actionBPM_Averaging = QtWidgets.QAction(MainWindow)
        self.actionBPM_Averaging.setCheckable(True)
        self.actionBPM_Averaging.setChecked(True)
        self.actionBPM_Averaging.setObjectName("actionBPM_Averaging")
        self.actionLicense = QtWidgets.QAction(MainWindow)
        self.actionLicense.setObjectName("actionLicense")
        self.WindowSizeGroup = QtWidgets.QActionGroup(MainWindow)
        self.WindowSizeGroup.setObjectName("WindowSizeGroup")
        self.action2000 = QtWidgets.QAction(self.WindowSizeGroup)
        self.action2000.setCheckable(True)
        self.action2000.setObjectName("action2000")
        self.action5000 = QtWidgets.QAction(self.WindowSizeGroup)
        self.action5000.setCheckable(True)
        self.action5000.setObjectName("action5000")
        self.action8000 = QtWidgets.QAction(self.WindowSizeGroup)
        self.action8000.setCheckable(True)
        self.action8000.setChecked(True)
        self.action8000.setObjectName("action8000")
        self.action10000 = QtWidgets.QAction(self.WindowSizeGroup)
        self.action10000.setCheckable(True)
        self.action10000.setObjectName("action10000")
        self.FPSGroup = QtWidgets.QActionGroup(MainWindow)
        self.FPSGroup.setObjectName("FPSGroup")
        self.action30_FPS = QtWidgets.QAction(self.FPSGroup)
        self.action30_FPS.setCheckable(True)
        self.action30_FPS.setChecked(True)
        self.action30_FPS.setObjectName("action30_FPS")
        self.action15_FPS = QtWidgets.QAction(self.FPSGroup)
        self.action15_FPS.setCheckable(True)
        self.action15_FPS.setChecked(False)
        self.action15_FPS.setObjectName("action15_FPS")
        self.action8_FPS = QtWidgets.QAction(self.FPSGroup)
        self.action8_FPS.setCheckable(True)
        self.action8_FPS.setObjectName("action8_FPS")
        self.menuFile.addAction(self.actionQuit)
        self.menuRefresh_Rate.addAction(self.action30_FPS)
        self.menuRefresh_Rate.addAction(self.action15_FPS)
        self.menuRefresh_Rate.addAction(self.action8_FPS)
        self.menuExport.addAction(self.menuRefresh_Rate.menuAction())
        self.menuExport.addAction(self.actionBold_Line)
        self.menuHelp.addAction(self.actionAbout)
        self.menuHelp.addAction(self.actionLicense)
        self.menuHelp.addSeparator()
        self.menuHelp.addAction(self.actionGet_Source_Code)
        self.menuExport_2.addAction(self.actionPNG)
        self.menuExport_2.addAction(self.actionCSV)
        self.menuExport_2.addAction(self.actionRAW)
        self.menuWindow_Size.addAction(self.action2000)
        self.menuWindow_Size.addAction(self.action5000)
        self.menuWindow_Size.addAction(self.action8000)
        self.menuWindow_Size.addAction(self.action10000)
        self.menuCapture.addAction(self.actionStart_Stop)
        self.menuCapture.addAction(self.actionReset)
        self.menuCapture.addSeparator()
        self.menuCapture.addAction(self.menuWindow_Size.menuAction())
        self.menuCapture.addAction(self.actionBPM_Averaging)
        self.menuCapture.addAction(self.actionAuto_Holdoff)
        self.menuBar.addAction(self.menuFile.menuAction())
        self.menuBar.addAction(self.menuCapture.menuAction())
        self.menuBar.addAction(self.menuExport.menuAction())
        self.menuBar.addAction(self.menuExport_2.menuAction())
        self.menuBar.addAction(self.menuHelp.menuAction())

        self.retranslateUi(MainWindow)
        QtCore.QMetaObject.connectSlotsByName(MainWindow)

    def retranslateUi(self, MainWindow):
        _translate = QtCore.QCoreApplication.translate
        MainWindow.setWindowTitle(_translate("MainWindow", "ECG Viewer "))
        self.button_connect.setText(_translate("MainWindow", "Connect"))
        self.button_refresh.setText(_translate("MainWindow", "Refresh"))
        self.label_2.setText(_translate("MainWindow", "Device:"))
        self.label_3.setText(_translate("MainWindow", "Alarm Control:"))
        self.label_4.setText(_translate("MainWindow", "High Limit:"))
        self.label_5.setText(_translate("MainWindow", "Low Limit:"))
        self.alarm_text.setText(_translate("MainWindow", "RATE ALARM"))
        self.label.setText(_translate("MainWindow", "Approximate BPM"))
        self.show_track.setText(_translate("MainWindow", "Show signal tracking"))
        self.button_reset.setText(_translate("MainWindow", "Reset"))
        self.button_run.setText(_translate("MainWindow", "Stop"))
        self.label_6.setText(_translate("MainWindow", "Advanced Controls"))
        self.label_8.setText(_translate("MainWindow", "PD - Holdoff"))
        self.label_9.setText(_translate("MainWindow", "PD - Prominence"))
        self.label_10.setText(_translate("MainWindow", "Filtering Window Length"))
        self.label_11.setText(_translate("MainWindow", "Filtering Polyorder"))
        self.autoinvert_checkbox.setText(_translate("MainWindow", "Auto Invert"))
        self.button_ui_force_invert.setText(_translate("MainWindow", "Force Invert"))
        self.label_7.setText(_translate("MainWindow", "Vertical Zoom"))
        self.label_12.setText(_translate("MainWindow", "Hello World"))
        self.menuFile.setTitle(_translate("MainWindow", "File"))
        self.menuExport.setTitle(_translate("MainWindow", "Graph"))
        self.menuRefresh_Rate.setTitle(_translate("MainWindow", "Refresh Rate"))
        self.menuHelp.setTitle(_translate("MainWindow", "Help"))
        self.menuExport_2.setTitle(_translate("MainWindow", "Export"))
        self.menuCapture.setTitle(_translate("MainWindow", "Capture"))
        self.menuWindow_Size.setTitle(_translate("MainWindow", "Window Size"))
        self.actionQuit.setText(_translate("MainWindow", "Quit"))
        self.actionAbout.setText(_translate("MainWindow", "About"))
        self.actionBold_Line.setText(_translate("MainWindow", "Bold Line"))
        self.actionPNG.setText(_translate("MainWindow", "PNG"))
        self.actionCSV.setText(_translate("MainWindow", "CSV"))
        self.actionRAW.setText(_translate("MainWindow", "RAW"))
        self.actionGet_Source_Code.setText(_translate("MainWindow", "Get Source Code"))
        self.actionCapture_Rate.setText(_translate("MainWindow", "Capture Window Size"))
        self.actionReset.setText(_translate("MainWindow", "Reset"))
        self.actionStart_Stop.setText(_translate("MainWindow", "Start / Stop"))
        self.actionAuto_Holdoff.setText(_translate("MainWindow", "Auto Holdoff"))
        self.actionBPM_Averaging.setText(_translate("MainWindow", "BPM Averaging"))
        self.actionLicense.setText(_translate("MainWindow", "License"))
        self.action2000.setText(_translate("MainWindow", "2000"))
        self.action5000.setText(_translate("MainWindow", "5000"))
        self.action8000.setText(_translate("MainWindow", "8000"))
        self.action10000.setText(_translate("MainWindow", "10000"))
        self.action30_FPS.setText(_translate("MainWindow", "30 FPS"))
        self.action15_FPS.setText(_translate("MainWindow", "15 FPS"))
        self.action8_FPS.setText(_translate("MainWindow", "8 FPS"))
        self.simulatorControlsGroup.setTitle(_translate("MainWindow", "ECG Simulator"))
        self.dataSourceLabel.setText(_translate("MainWindow", "Data Source:"))
        self.useSerialRadio.setText(_translate("MainWindow", "Serial Input"))
        self.useGeneratorRadio.setText(_translate("MainWindow", "ECG Generator"))
        self.ecgTypeLabel.setText(_translate("MainWindow", "ECG Type:"))
        self.heartRateLabel.setText(_translate("MainWindow", "Heart Rate (BPM):"))
        self.currentHeartRateLabel.setText(_translate("MainWindow", "75"))
        self.noiseLabel.setText(_translate("MainWindow", "Noise Level:"))
        self.currentNoiseLabel.setText(_translate("MainWindow", "0.10"))
from pyqtgraph import PlotWidget
